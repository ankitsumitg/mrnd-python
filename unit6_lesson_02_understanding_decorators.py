...
def test_decorators_arguments():
    ...
    def runif(variable):
        def log(func):
            def inner():
                if (os.environ.__contains__(variable) and os.environ[ variable ] == '1'):
                    return func()
                else:
                    pass
            return inner
...
def test_decorators_wraps():
    ...
    # modify this decorator to preserve __name__ and __doc__ by copying them over to the wrapper function.
    def pass_through2(func):
        def inner(*args, **kwargs):
            func(*args, **kwargs)
        return inner
    ...
    assert 'inner' == sub.__name__
    assert None == sub.__doc__
    assert None == sub(20, 10)
...
def test_decorators_custom_wrap():
    ...
    assert None  == add(10, 20)


three_things_i_learnt = """
-How decoraters works
-How to return fuctions
-Call via diffrent decorator
"""
