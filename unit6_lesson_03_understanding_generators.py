...
def test_evens():
    ...
    for num in range(1000):
        if num%2 == 0: assert num == next(gen)

    for num in range(1000):
        if num % 2 == 0: assert num+1000 == next(gen)
...
def fizz_buzz():
    cur = 1
    while True:
        if cur%15 == 0:yield (cur, "fizz", "buzz")
        elif cur%3 == 0:yield (cur, "fizz")
        elif cur % 5 == 0: yield (cur, "buzz")
        else: yield (cur, None)
        cur += 1
    pass
...
-How generators work
-how to make our wone generator
