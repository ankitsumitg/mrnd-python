...
def get_lower_to_upper_dict():
    ...
    return dict(zip(list(map(chr,lower)),list(map(chr,upper))))
...
def get_encoding_dict():
    ...
    upper = list(range(ord('A'), ord('Z') + 1))
    #new syntax
    return {**dict(zip(list(map(chr,lower)),list(range(1,27)))),**dict(zip(list(map(chr,upper)),list(range(1,27))))}
...
def cap(name): return name[:1].upper() + name[1:].lower()
...
def bit_count_even(number):
    ...
    """
    check = [True,False]
    return check[bin(number).count('1')%2]
