...
def custom_sort(input):
    if input == None or input == [ ]: return input
    from operator import itemgetter
    #input = sorted(sorted(input, key=itemgetter(2)), key=itemgetter(1), reverse=True)
    input.sort(key=itemgetter(2))
    input.sort(key=itemgetter(1),reverse = True)
    return input
...
def max_vowels(words):
    if words == None or words == []:return None
    input = list(words)
    i = [ ]
    for s in input:
        i.append(len((VOWELS-set(s.lower())).symmetric_difference(VOWELS)))
    return words[i.index(max(i))]
def test_max_vowels():
