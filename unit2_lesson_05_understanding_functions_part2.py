...
def test_function_call_with_keyword_arguments():
    assert [10,2,3,4] == demo(10,2) #1 extra positional argument required
    assert [10,20,3,4] == demo(10, 20)
    assert [10,20,30,4] == demo(10, 20, 30)
    assert [1,20,3,4] == demo(1,second=20)
    assert [1,20,30,4] == demo(1,second=20, third=30)#required additonal parameter
    assert [10,2,30,4] == demo(first=10,second=2, third=30)
    assert [10,2,30,4] == demo(10,second=2, third=30)
    # assert [__] == demo(second=20, 10)  # is this allowed? correct and uncomment
...
def simple_format(format, *args):
    ...
    for i in xrange(len(args)):
        format = format.replace("%" + str(i), args[ i ])
    return format
