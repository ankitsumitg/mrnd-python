...
NOTE: For this assignment define functions in a module called factormath which lives in unit3tests package.
...
def test_get_hcf():
    ...
    from unit3tests.factormath import get_hcf
    assert get_hcf([], [(2,5)]) == []
    assert get_hcf([], []) == []
...
def test_get_lcm():
    # define the right packages and modules so that this import works
    from unit3tests.factormath import get_lcm
